...
INSERT INTO `Users`(`userId`,`userName`,`userBirth`,`userGender`,`userHp`,`userPoint`,`userLevel`,`userAddr`,`userRegDate`) 
				VALUES ('user2','김유신','1975-06-11','M','010-1102-1975',1000,1,'서울','2022-01-10 10:50:12');
INSERT INTO `Users`(`userId`,`userName`,`userBirth`,`userGender`,`userHp`,`userPoint`,`userLevel`,`userAddr`,`userRegDate`) 
...
INSERT INTO `Users` VALUES ('user6','선덕여왕','1979-07-28','F','010-3101-1979','queen@naver.com',1600,2,'대전','2022-01-10 10:50:12');
...
INSERT INTO `Points` (`userId`,`point`,`pointDesc`,`pointDate`) VALUES ('user2',2000,'이벤트 응모 2000적립','2022-01-10 10:50:12');
...
INSERT INTO `Points` (`userId`,`point`,`pointDesc`,`pointDate`) VALUES ('user6',15840,'상품구매 5% 적립','2022-01-10 10:50:12');
...
INSERT INTO `Products` (`prodNo`,`cateNo`,`SellerNo`,`prodName`,`prodPrice`,`prodSold`,`prodDiscount`) 
					VALUES (120101,12,10010,'암소1등급 구기셋트 1.2kg',150000,87,15);
INSERT INTO `Products` (`prodNo`,`cateNo`,`SellerNo`,`prodName`,`prodPrice`,`prodSold`,`prodDiscount`) 
...
INSERT INTO `Products` (`prodNo`,`cateNo`,`SellerNo`,`prodName`,`prodPrice`,`prodSold`,`prodDiscount`) 
					VALUES (141001,14,10001,'라뗴 2인 소파/방수패브릭',320000,42,0);
...
INSERT INTO `Orders` VALUES ('22010310001','user5',127000,'경기도 광주시초월로 21',1,'2022-01-10 10:50:12');
...
##문제1 모든 장바구니 내역에서 고객명, 상품명, 상품수량을 조회하시오. 단 상품수량 2건 이상만 조회할것
...
##문제2 모든 상품내역에서 상품번호, 상품카테고리명, 상품명 ,상품가격 판매사이트이름, 판매자 연락처를 조회하시오
SELECT `prdoNo`,`cateName`,`prodName`,`prodPrice`,`sellerManager`,`sellerPhone` FROM `Products` AS a
JOIN`Categories` AS b ON a.cateNo = b.cateNo
JOIN`Sellers` AS c ON a.sellerNo = c.sellerNo;

##문제3 모든 고객의 아이디, 이름, 휴대폰, 현재포인트, 적립포인트 총합을 조회하시오. 단 적립포이트 내역이 없으면 0으로 출력
...
##문제4. 모든 주문의 주문번호,주문자 아이디, 고객명, 주문가격, 주문일자를 조회하시오. 
	    ##단 주문 금액에 10만원 이상, 큰 금금액순으로 조회, 금액이 같으면 이름이 사전순으로 될것
...
##문제5. 모든 주문의 주문번호, 주문사 아이디 , 고객명, 상품명, 주문일자를 조회하시오. 주문번호는 중복없이 상품명 은 구분자로 나열할것
SELECT 	
	a.`orderNo`,
	ANY_VALUE(b.`userId`),
	ANY_VALUE(b.`userName`),
...
##문제6. 모든상품의 상품번호,상품명,상품가격,할인율,할인된 가격을 조회하시오.
SELECT `prodNo`,`prodName`,`prodPrice`,`ProdDiscount`,`prodPrice`,
FLOOR(`prodprice` * (1-`prodDiscount`/ 100)) AS '할인된가격' FROM `Products` AS a;
...
#문제8 아직 상품을 판매하지 않은 판매자의 판매자 번호 , 판매자 상호, 판매자이름,판매자연락처를 조회하시오 
SELECT a.sellerNo,
	a.sellerBizName,
	a.sellerManager,
	a.sellerPhone FROM `Sellers` AS a
LEFT JOIN `Products` AS b ON a.sellerNo =b.sellerNo
WHERE `prodNo` IS NULL;


#문제 9 모든 주문상세내역 중 개별 상품 가격과 갯수 그리고 할인율이 적용된  종합을 구하고 
## 주문별 총합을 구해서 주문별 총합이 10만원이상 그리고 큰금액순으로 `주문번호, 최종총합을 구하시요 
SELECT
	`orderNo`,
	SUM(`할인가`) AS `최종 총합`
FROM
...
FROM `OrderItems`
 ...
 ##문제 10 장보고 고객이 주문했던 모든 상품을 `고객명`,`상품명`으로 조회하시오.  
 #단 상품명은 중복 안되므 상품며은 구분자, 로 나뉨
SELECT 
	`userName`,
