...
INSERT INTO `Departments` VALUES ('111','안과','김박지원','051-123-0111');
...
##문제 6  모든 진료내역에서 '화상'에서 진료한 내역 가운데 진료번호, 환자이름, 담당의사명, 진료내용 ,처방내용 ,진료날짜를 조회하시오.
SELECT a.`treat_no`,`pat_name`,`doc_name`,`treat_contents`,`chart_contents`,`treat_datetime` FROM `Treatments` AS a
JOIN `Doctors` AS b ON a.doc_id = b.doc_id
JOIN `Patients` AS c ON a.pat_id = c.pat_id
JOIN `Charts` AS d ON a.treat_no = d.treat_no
...
##문제 7 현재 날짜 기준으로 30세이상 ~40세 미만 환자를 조회하시오
SELECT * FROM
	(SELECT *,
...
SELECT `dep_name`,`dep_name` FROM `Departments` AS a
LEFT JOIN `Doctors` AS b ON a.dep_no = b.dep_no
WHERE `doc_id` IS NULL;
##문제9 김태희 간호사가 맡고있는 모든 환자 이름을 구분자 ^으로 해서 '강동원^고소영^이나영' 과 같이 조회하시오
SELECT 
...
##문제 10 가장 많은 환자 수를 담당하는 간호사 ID, 간호사 이름 ,담당 환자수를 조회하시오 .
SELECT a.`nur_id`,`nur_name`,COUNT(*) AS `환자수` FROM `Patients` AS a
JOIN `Nurses` AS b ON a.nur_id = b.nur_id
GROUP BY a.`nur_id`
ORDER BY `환자수` DESC
LIMIT 1;
